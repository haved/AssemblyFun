...
public class TaskDatabaseOpenHelper extends SQLiteOpenHelper {
    ...
    public void addDefaultValues(SQLiteDatabase db) {
        ...
        long task1 = addTaskInfoToTables(db, values, "Tutorial task 1: Running", "Run your application by pressing the green run button", System.currentTimeMillis(), Difficulty.TUTORIAL, 4.4f, "Haved", true, 11111111);
        addTaskInfoToTables(db, values, "Tutorial task 2: Returning", "Return the number 10 by using 'mov r0, #10'\nr0 is a registry, #10 is a number", System.currentTimeMillis(), Difficulty.TUTORIAL, 4.7f, "Haved", true, 22002222);
        addTaskInfoToTables(db, values, "Tutorial task 3: Adding", "Use 'add r0,#5,#6' to store 5+6 in r0\nAll registries store numbers", System.currentTimeMillis(), Difficulty.TUTORIAL, 4.1f, "Haved", true, 30000003);
        addTaskInfoToTables(db, values, "Tutorial task 4: Breaking", "You can write 'mov pc, lr' to exit!\npc and lr are special registries", System.currentTimeMillis(), Difficulty.TUTORIAL, 4.8f, "Haved", false, 10004444);
        addTaskInfoToTables(db, values, "Tutorial task 5: Comparing", "Use 'cmp r0,#5' to compare r0 and 5\nThe comparison is stored in memory", System.currentTimeMillis(), Difficulty.TUTORIAL, 4.7f, "Haved", true, 55550055);
        addTaskInfoToTables(db, values, "Tutorial task 6: Comparing#2", "Use 'cmp r1,#5' to compare r0 and 5\n'movlt r0,#7' will move 7 into r0 only if r1 is less than (lt) 5", System.currentTimeMillis(), Difficulty.TUTORIAL, 4.2f, "Haved", false, 100666);
        long task7 = addTaskInfoToTables(db, values, "Tutorial task 7: Test", "Write a program that compares r0 and r1 and stores the biggest value in r0", System.currentTimeMillis(), Difficulty.VERY_EASY, 4.4f, "Haved", false, 77744777);
        addTaskInfoToTables(db, values, "Tutorial task 8: Test2", "If r0 is greater than(-gt) r1, subtract (sub) r1 from r0 and store it in r0", System.currentTimeMillis(), Difficulty.VERY_EASY, 4.6f, "Haved", false, 8888988);

        addTaskToLocalTasks(db, values, task1, "Hit the green button to run your program. A series of test are run to check if your program performs to specification.",
